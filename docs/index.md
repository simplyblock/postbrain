...
| `designs/DESIGN_CROSS_SCOPE_VERIFICATION.md` | `designs/TASKS.md` | Dedicated cross-scope verification retrieval flow for comparing baseline scope claims (for example docs) against evidence in source/session/knowledge scopes with strict provenance and time-window filters. | You add or change `verify_context`/cross-scope recall behavior, per-scope comparison semantics, provenance output contracts, or time-windowed verification logic. |

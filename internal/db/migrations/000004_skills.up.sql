...
-- ─────────────────────────────────────────
CREATE TABLE {{POSTBRAIN_SCHEMA}}.skills (
    ...
    source_artifact_id UUID REFERENCES {{POSTBRAIN_SCHEMA}}.knowledge_artifacts(id) ON DELETE SET NULL,
    ...
    previous_version   UUID REFERENCES {{POSTBRAIN_SCHEMA}}.skills(id),
...
CREATE INDEX skills_embedding_hnsw_idx
    ON {{POSTBRAIN_SCHEMA}}.skills USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

CREATE INDEX skills_scope_status_idx
    ON {{POSTBRAIN_SCHEMA}}.skills (scope_id, status, visibility);

CREATE INDEX skills_content_fts_idx
    ON {{POSTBRAIN_SCHEMA}}.skills USING GIN (to_tsvector('postbrain_fts', description || ' ' || body));

-- ─────────────────────────────────────────
-- 2. Skill endorsements
-- ─────────────────────────────────────────
CREATE TABLE {{POSTBRAIN_SCHEMA}}.skill_endorsements (
    id          UUID PRIMARY KEY DEFAULT uuidv7(),
...
-- ─────────────────────────────────────────
CREATE TABLE {{POSTBRAIN_SCHEMA}}.skill_history (
    id          UUID PRIMARY KEY DEFAULT uuidv7(),
    skill_id    UUID NOT NULL REFERENCES {{POSTBRAIN_SCHEMA}}.skills(id) ON DELETE CASCADE,
...
-- ─────────────────────────────────────────
CREATE TRIGGER skills_updated_at BEFORE UPDATE ON {{POSTBRAIN_SCHEMA}}.skills
...
-- ─────────────────────────────────────────
CREATE OR REPLACE FUNCTION {{POSTBRAIN_SCHEMA}}.skills_update_invocation_stats()
...
CREATE TRIGGER events_skill_stats
    AFTER INSERT ON {{POSTBRAIN_SCHEMA}}.events
    FOR EACH ROW EXECUTE FUNCTION {{POSTBRAIN_SCHEMA}}.skills_update_invocation_stats();

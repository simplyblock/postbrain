...
-- ─────────────────────────────────────────
CREATE TABLE artifact_digest_sources (
    digest_id   UUID NOT NULL REFERENCES knowledge_artifacts(id) ON DELETE CASCADE,
...
CREATE INDEX artifact_digest_sources_source_idx
    ON artifact_digest_sources (source_id);

-- ─────────────────────────────────────────
-- 3. Synthesis audit log
-- ─────────────────────────────────────────
CREATE TABLE knowledge_digest_log (
    id              UUID PRIMARY KEY DEFAULT uuidv7(),
    scope_id        UUID NOT NULL REFERENCES scopes(id) ON DELETE CASCADE,
    digest_id       UUID NOT NULL REFERENCES knowledge_artifacts(id) ON DELETE CASCADE,
    source_ids      UUID[] NOT NULL,
    strategy        TEXT NOT NULL CHECK (strategy IN ('manual', 'auto_cluster')),
    synthesised_by  UUID REFERENCES principals(id),
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX knowledge_digest_log_digest_idx ON knowledge_digest_log (digest_id);
CREATE INDEX knowledge_digest_log_scope_idx  ON knowledge_digest_log (scope_id, created_at DESC);

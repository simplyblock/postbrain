...
// TestPromotionAccess_DirectPath_KnowledgeWriteAlone verifies that knowledge:write
// alone (without memories:write) is sufficient for PathDirect.
func TestPromotionAccess_DirectPath_KnowledgeWriteAlone(t *testing.T) {
	...
	if kind != authz.PathDirect {
		t.Errorf("kind = %v, want PathDirect (knowledge:write alone should qualify)", kind)
	}
	if reviewRequired != authz.StandardReviewRequired {
		t.Errorf("reviewRequired = %d, want StandardReviewRequired (%d)", reviewRequired, authz.StandardReviewRequired)
	}

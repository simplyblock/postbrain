...
func (h *Handler) graphViewData(r *http.Request) struct {
	...
	ents, err := compat.ListEntitiesByScope(r.Context(), h.pool, scope.ID, "", 100000, 0)
	if err == nil {
		for _, e := range ents {
	...
	if rels, err := compat.ListRelationsByScope(r.Context(), h.pool, scope.ID); err == nil {

...
func (s *Server) registerCrossScopeContext() {
	s.mcpServer.AddTool(mcpgo.NewTool("cross_scope_context",
		...
		mcpgo.WithArray("comparison_scopes",
			mcpgo.WithStringItems(),
			mcpgo.Description("Comparison scopes as kind:external_id"),
		),
		mcpgo.WithArray("layers",
			mcpgo.WithStringEnumItems([]string{"memory", "knowledge"}),
			mcpgo.Description("Layers to query: memory|knowledge (default: both)"),
		),
